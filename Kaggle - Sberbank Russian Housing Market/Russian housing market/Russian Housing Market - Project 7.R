...
## Model 8
glm.gaussian.model8 <- glm(formula = I(log(price_doc))~full_sq:floor+sub_area,family = gaussian,data = traindata.new)

summary(glm.gaussian.model8)
par(mfrow = c(2,2))
plot(glm.gaussian.model8)

## RMSLE
antilog.target <- exp(glm.gaussian.model8$fitted.values)
...
rmsle.glm.gaussian.model8 <- (sum(val,na.rm = T)/n)^(1/2)
rmsle.glm.gaussian.model8 # 0.5433218
## RMSLE ends

## Prediction
## rebuilding the model after removing product_type column
preds.glm.gaussian.model8 <- predict(object = glm.gaussian.model8, newdata = testdata.new)
sum(is.na(preds.glm.gaussian.model8))

## File
submission8 <- cbind.data.frame(id=test.srhm$id,price_doc = exp(preds.glm.gaussian.model8))
write.csv(submission8,paste0(dir.kaggle.srhm,"/submissions/submission8.csv"),row.names = F)


## Model 9
glm.gaussian.model9 <- glm(formula = I(log(price_doc))~.*.,family = gaussian,data = traindata.new)

summary(glm.gaussian.model9)
par(mfrow = c(2,2))
plot(glm.gaussian.model9)

## RMSLE
antilog.target <- exp(glm.gaussian.model9$fitted.values)
...
rmsle.glm.gaussian.model9 <- (sum(val,na.rm = T)/n)^(1/2)
rmsle.glm.gaussian.model9 # 0.4796737
## RMSLE ends

## Prediction
## rebuilding the model after removing product_type column
preds.glm.gaussian.model9 <- predict(object = glm.gaussian.model9, newdata = testdata.new)
sum(is.na(preds.glm.gaussian.model9))

## File
submission9 <- cbind.data.frame(id=test.srhm$id,price_doc = exp(preds.glm.gaussian.model9))
write.csv(submission9,paste0(dir.kaggle.srhm,"/submissions/submission9.csv"),row.names = F)
formula(glm.gaussian.model9)